 ...
 */
export async function validateCouponCode(prisma, { code, promotion: passedPromotion, userId, resortId, originalAmount }) {
  let promotion = passedPromotion;
  ...
  if (userId) {
    ...
    if (promotion.firstBookingOnly) {
      const previousBookingsCount = await prisma.booking.count({ 
        where: { 
          userId, 
          status: { in: ['CONFIRMED', 'PAID', 'COMPLETED', 'CHECKED_IN'] } 
        } 
      });
...
export async function findBestAutoApplyCoupon(prisma, { userId, resortId, originalAmount }) {
  ...
  for (const promo of activePromos) {
    const res = await validateCouponCode(prisma, { promotion: promo, code: promo.code || promo.name, userId, resortId, originalAmount });
