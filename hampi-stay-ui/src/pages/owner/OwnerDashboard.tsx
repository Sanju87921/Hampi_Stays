...
export function OwnerDashboard() {
  ...
  const handleDownloadInvoice = async (booking: any) => {
    ...
    doc.text("PARTNER NETWORK INVOICE", 20, 38);

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(10);
    doc.setFont("helvetica", "bold");
    doc.text("BUSINESS RECEIPT", 140, 25);
    doc.setFont("helvetica", "normal");
    doc.text(`REF: ${safeRef}`, 140, 31);
    doc.text(`DATE: ${new Date().toLocaleDateString("en-IN")}`, 140, 37);

    // 2. Main Info
    doc.setTextColor(navy[0], navy[1], navy[2]);
    doc.setFontSize(14);
    doc.setFont("serif", "bold");
    doc.text("GUEST & STAY INFO", 20, 70);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Guest: ${booking.user?.name}`, 20, 78);
    doc.text(`Email: ${booking.user?.email}`, 20, 83);
    doc.text(`Resort: ${resort.name}`, 20, 88);

    doc.setFontSize(14);
    doc.setFont("serif", "bold");
    doc.text("BOOKING DETAILS", 110, 70);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Check-in: ${new Date(booking.checkIn).toLocaleDateString("en-IN")}`, 110, 78);
    doc.text(`Check-out: ${new Date(booking.checkOut).toLocaleDateString("en-IN")}`, 110, 83);
    doc.text(`Guests: ${booking.guests} People`, 110, 88);
    ...
    autoTable(doc, {
      ...
      body: [
        ['Accommodation Payout', `INR ${booking.totalPrice?.toLocaleString("en-IN")}`],
        ['Service Fee (Included)', '7% Platform Fee'],
        ['Total Revenue (Net)', `INR ${(booking.totalPrice * 0.93).toLocaleString("en-IN")}`],
      ],
    ...
    try {
      const qrData = `Ref: ${safeRef} | Guest: ${booking.user?.name} | Property: ${resort.name}`;
      const qrCode = await QRCode.toDataURL(qrData);
      doc.addImage(qrCode, 'PNG', 140, tableFinalY + 10, 40, 40);
      doc.setFontSize(9);
      doc.text("Verified Partner Check-in", 140, tableFinalY + 55);
    } catch (e) { console.error(e); }

    // 5. Footer
    doc.setFontSize(8);
    doc.setTextColor(150, 150, 150);
    doc.text("Automated Invoice Generated by HampiStays Partner Portal.", 105, 285, { align: 'center' });
