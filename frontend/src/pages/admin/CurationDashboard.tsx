...
const CurationDashboard = () => {
 ...
 <p className="text-xs text-navy-950 mb-4">Expires: Dec 31, 2026</p>
 <button className="w-full py-2 border border-sand-300 rounded-xl text-xs font-bold text-navy-950 hover:bg-sand-100 :bg-sand-200 transition-colors">Replace Resort</button>
 </div>
 </div>
 <div className="bg-sand-50 rounded-2xl border border-sand-200 overflow-hidden">
 <div className="h-40 bg-sand-200 relative">
 ...
 <p className="text-xs text-navy-950 mb-4">Expires: Nov 15, 2026</p>
 <button className="w-full py-2 border border-sand-300 rounded-xl text-xs font-bold text-navy-950 hover:bg-sand-100 :bg-sand-200 transition-colors">Replace Resort</button>
 </div>
 </div>
 <div className="bg-sand-50 rounded-2xl border-2 border-dashed border-sand-200 flex flex-col items-center justify-center p-6 h-[280px] hover:bg-sand-50 :bg-sand-100 transition-colors cursor-pointer group">
 ...
 <p className="text-xs text-navy-950 text-center max-w-[150px]">Select a resort to feature on the homepage.</p>
 </div>
 </div>
 </div>
 );
 ...
 <Link to="/dashboard" className="text-xs font-bold uppercase tracking-widest text-navy-950 hover:text-gold-600 transition-colors">Dashboard</Link>
 ...
 <p className="text-navy-950 max-w-2xl">Govern the platform's visual hierarchy, manage sponsored placements, and control the luxury brand narrative.</p>
 </div>
 <div className="flex items-center gap-4 bg-white backdrop-blur-md px-6 py-3 rounded-2xl border border-sand-200 shadow-sm">
 ...
 className={`flex items-center gap-3 w-full px-4 py-3.5 rounded-2xl transition-all duration-300 font-semibold text-sm ${activeTab === tab.id ? 'bg-navy-950 text-white shadow-md' : 'text-navy-950 hover:bg-sand-50 :bg-sand-100 hover:text-navy-950 '}`}>
 <span className={activeTab === tab.id ? 'text-gold-400' : 'text-navy-950 '}>{tab.icon}</span>
 {tab.label}
 </button>
 ))}
 </div>
 ...
 <div className="p-8 border-b border-sand-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-sand-50 rounded-t-[2.5rem]">
 <div>
 <h2 className="text-2xl font-serif font-bold text-navy-950 mb-1">{tabs.find(t => t.id === activeTab)?.label}</h2>
 <p className="text-sm text-navy-950 ">Configure and manage active placements.</p>
 </div>
 </div>
