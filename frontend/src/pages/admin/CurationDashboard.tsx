...
import { Sparkles, Crown, Image as ImageIcon, Map, Search, Settings, ShieldCheck, ChevronRight, Megaphone, Calendar as CalendarIcon, Star } from 'lucide-react';
        ...
        >
          <div>
            <div className="flex items-center gap-2 mb-2">
              <Link to="/dashboard" className="text-xs font-bold uppercase tracking-widest text-navy-950/40 hover:text-gold-600 transition-colors">
                Dashboard
              </Link>
            ...
            <p className="text-navy-950/60 max-w-2xl">
              Govern the platform's visual hierarchy, manage sponsored placements, and control the luxury brand narrative through curated collections.
            </p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="p-3 flex flex-col gap-1">
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`flex items-center gap-3 w-full px-4 py-3.5 rounded-2xl transition-all duration-300 font-semibold text-sm ${
                      activeTab === tab.id 
                        ? 'bg-navy-950 text-white shadow-md' 
                        : 'text-navy-950/60 hover:bg-sand-50 hover:text-navy-950'
                    }`}
                  >
                    <span className={`${activeTab === tab.id ? 'text-gold-400' : 'text-navy-950/40'}`}>
                      {tab.icon}
                    </span>
                    {tab.label}
                  </button>
          ...
          <div className="flex-1">
            ...
            >
              ...
              {/* Header */}
              <div className="p-8 border-b border-sand-100 flex justify-between items-center bg-sand-50/20 rounded-t-[2.5rem]">
                <div>
                ...
                <button className="bg-navy-950 hover:bg-gold-600 text-white px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-widest transition-colors shadow-lg">
                  Save Changes
                </button>
              </div>
              ...
              <div className="p-8 flex-1 flex flex-col">
                ...
                {/* Coming Soon State (Simulating backend pending for features) */}
                {activeTab !== 'featured' ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center max-w-md mx-auto py-12">
                    ...
                    <p className="text-navy-950/60 text-sm mb-6">
                      The backend logic for {tabs.find(t => t.id === activeTab)?.label} is currently being upgraded for enterprise scale. It will be available shortly.
                    </p>
                    <span className="px-4 py-1.5 bg-gold-100 text-gold-700 text-[10px] font-bold uppercase tracking-widest rounded-full">
                      Coming Soon
                    </span>
                  </div>
                ) : (
                  /* Featured Resorts Content UI Placeholder */
                  <div className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                      
                      {/* Placeholder Card 1 */}
                      <div className="bg-sand-50 rounded-2xl border border-sand-200 overflow-hidden group">
                        ...
                        <div className="p-5">
                          ...
                          <button className="w-full py-2 border border-sand-300 rounded-xl text-xs font-bold text-navy-950 hover:bg-sand-100 transition-colors">
                            Replace Resort
                          </button>
                        </div>
                      </div>
                      ...
                      {/* Empty Slot */}
                      <div className="bg-sand-50/50 rounded-2xl border-2 border-dashed border-sand-200 overflow-hidden flex flex-col items-center justify-center p-6 h-[280px] hover:bg-sand-50 transition-colors cursor-pointer group">
