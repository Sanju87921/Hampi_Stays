        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className="bg-white hover:bg-sand-50 text-navy-950 border border-sand-200 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest transition-colors shadow-sm flex items-center gap-2 disabled:opacity-50"
            >
        ...
        <div className="space-y-3">
          {slides.length === 0 ? (
            ...
            slides.map((slide, idx) => (
              <motion.div key={slide.id} layout className={`flex flex-col sm:flex-row sm:items-center gap-4 p-3 rounded-2xl border transition-all ${slide.isActive ? 'bg-white border-sand-200' : 'bg-sand-50/50 border-sand-100 opacity-70'}`}>
                {/* Thumbnail */}
                  ...
                  {editingId === slide.id ? (
                    ...
                    <div>
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button onClick={() => startEdit(slide)} className="p-1.5 text-navy-950/40 hover:text-gold-600 bg-sand-50 hover:bg-sand-100 rounded-lg transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                      </div>
                      <p className="text-[10px] text-navy-950/40 mt-2 truncate font-mono bg-sand-50 px-1.5 py-0.5 rounded inline-block max-w-full">{slide.imageUrl}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0 sm:flex-col pt-2 sm:pt-0 border-t sm:border-t-0 sm:border-l border-sand-100 sm:pl-3">
                  <div className="flex gap-1 bg-sand-50 p-1 rounded-xl">
                    <button onClick={() => move(idx, -1)} disabled={idx === 0} className="p-1.5 rounded-lg hover:bg-white hover:shadow-sm disabled:opacity-30 transition-all"><ChevronUp className="w-4 h-4 text-navy-950/60" /></button>
                    <button onClick={() => move(idx, 1)} disabled={idx === slides.length - 1} className="p-1.5 rounded-lg hover:bg-white hover:shadow-sm disabled:opacity-30 transition-all"><ChevronDown className="w-4 h-4 text-navy-950/60" /></button>
                  </div>
                  <div className="flex gap-1">
                    <button onClick={() => toggle(slide)}
                      title={slide.isActive ? "Deactivate (Set as Draft)" : "Activate (Publish)"}
                      className={`p-2 rounded-xl transition-all shadow-sm ${slide.isActive ? 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100' : 'bg-white border border-sand-200 text-navy-950/40 hover:bg-sand-50'}`}>
