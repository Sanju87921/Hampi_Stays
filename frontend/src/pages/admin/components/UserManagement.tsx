...
import { Search, Shield, User, Hotel, MapPin, CheckCircle, XCircle, AlertTriangle, Trash2, BanIcon, RotateCcw } from 'lucide-react';
...
export function UserManagement() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      const data = await apiClient.get<any>(
        `/admin/users?role=${activeRole}&page=${page}&limit=10&search=${searchQuery}&showSuspended=${showSuspended}`
      );
      setUsers(data.users || []);
  ...
  }, [activeRole, page, searchQuery, showSuspended]);

  // Stage 1: Suspend (soft-delete, keeps record, blocks login)
  const handleSuspendUser = async (id: string, name: string) => {
    if (!(await confirm({ title: "Suspend User", message: `Suspend ${name}? They will be blocked from logging in but data is kept. You can hard-delete them afterwards.` }))) return;
    setProcessingId(id);
    try {
  ...
  // Stage 2: Hard Delete (permanently removes from DB — only for already-suspended users)
  const handleHardDeleteUser = async (id: string, name: string) => {
    if (!(await confirm({ title: "⚠️ Permanent Delete", message: `Permanently delete ${name} and ALL their data (bookings, resorts, reviews)? This CANNOT be undone.` }))) return;
    setProcessingId(id);
    try {
      await apiClient.delete(`/admin/users/${id}`);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[2.5rem] border border-sand-200 shadow-sm p-8 flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex gap-4 items-center">
          ...
          {/* Suspended toggle */}
          <button
            onClick={() => { setShowSuspended(s => !s); setPage(1); }}
            className={`flex items-center gap-2 px-4 py-3 rounded-2xl border text-sm font-bold transition-all ${
              showSuspended
                ? 'bg-red-50 text-red-600 border-red-200 hover:bg-red-100'
                : 'bg-sand-50 text-navy-950 border-sand-200 hover:bg-sand-100'
            }`}
          >
            <BanIcon className="w-4 h-4" />
            {showSuspended ? 'Viewing Suspended' : 'Show Suspended'}
          </button>
        </div>
      </div>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 rounded-full text-sm font-bold transition-all whitespace-nowrap ${
                  activeRole === tab.id
                    ...
                    : 'text-navy-950 hover:bg-sand-100 hover:text-navy-950 '
                }`}
              >
                {tab.icon}
                {tab.label}
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-[400px] text-navy-950 ">
              ...
              <p className="text-lg italic">
                {showSuspended ? `No suspended ${activeRole.toLowerCase()}s.` : `No active ${activeRole.toLowerCase()}s found.`}
              </p>
            </div>
          ) : (
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          {!showSuspended ? (
                            ...
                            <Button
                              variant="outline"
                              className="h-8 px-4 text-xs rounded-full border-orange-200 text-orange-600 hover:bg-orange-50 hover:text-orange-700 flex items-center gap-1"
                            ...
                            // Suspended users: show hard Delete button
                            <Button
                              variant="outline"
                              className="h-8 px-4 text-xs rounded-full border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 flex items-center gap-1"
