        ...
        <div className="bg-white rounded-[2.5rem] p-8 border border-sand-200 shadow-sm">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
            <div>
              ...
              <p className="text-sm text-navy-950/40">Manage SEO destination guides and organic marketing content.</p>
            </div>
            <div className="flex gap-4">
              <div className="relative">
                <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-navy-950/30" />
                <input 
                  type="text" 
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPosts.map(post => (
                <div key={post.id} className="bg-sand-50/50 border border-sand-100 rounded-2xl overflow-hidden hover:shadow-md transition-shadow group flex flex-col h-full">
                  <div className="h-40 relative overflow-hidden bg-sand-200">
                    {post.coverImage ? (
                      ...
                      <div className="flex items-center justify-center w-full h-full text-navy-950/20">
                  ...
                  <div className="p-6 flex-grow flex flex-col">
                    ...
                    <p className="text-sm text-navy-950/50 line-clamp-2 mb-6 flex-grow">{post.excerpt || post.content.replace(/<[^>]+>/g, '').substring(0, 100)}</p>
                    
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-sand-200/50">
                      <div className="flex gap-2">
                        <button onClick={() => handleOpenEditor(post)} className="w-10 h-10 rounded-xl bg-white border border-sand-200 text-navy-950 flex items-center justify-center hover:bg-sand-100 transition-colors" title="Edit">
                      ...
                      <a href={`/blog/${post.slug}`} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-[10px] font-bold text-navy-950/50 uppercase tracking-widest hover:text-gold-600 transition-colors">
            ...
            <div className="text-center py-20 bg-sand-50 rounded-2xl border border-dashed border-sand-200">
              <FileText className="w-12 h-12 text-navy-950/20 mx-auto mb-4" />
              <h4 className="text-lg font-bold text-navy-950 mb-2">No Articles Found</h4>
              <p className="text-navy-950/50 max-w-sm mx-auto mb-6">Create your first blog post to start driving organic traffic.</p>
        ...
        >
          ...
          <form onSubmit={handleSavePost} className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2 space-y-6">
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-2 block">Article Title *</label>
                <input 
                  type="text" 
                  required
                  value={title}
              ...
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-2 block">Content (HTML/Markdown) *</label>
                <textarea 
                  required
                  value={content}
              ...
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-2 block">Excerpt (Short Summary)</label>
                <textarea 
                  value={excerpt}
            ...
            <div className="space-y-6 bg-sand-50/50 p-6 rounded-2xl border border-sand-100">
              <div className="flex items-center justify-between bg-white p-4 rounded-xl border border-sand-200 shadow-sm">
                <div>
                  ...
                  <p className="text-[10px] text-navy-950/50 uppercase tracking-widest">{published ? "Live on site" : "Hidden draft"}</p>
                </div>
              ...
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-2 block">Cover Image URL</label>
                <input 
                  type="url" 
                  value={coverImage}
              ...
              <div className="pt-6 border-t border-sand-200 space-y-4">
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-1 block">Custom Slug</label>
                  <input 
                    type="text" 
                    value={slug}
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-1 block">Meta Title</label>
                  <input 
                    type="text" 
                    value={seoTitle}
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-1 block">Meta Description</label>
                  <textarea 
                ...
                <div>
                  <label className="text-[10px] font-bold uppercase tracking-widest text-navy-950/40 ml-1 mb-1 block">Keywords (comma separated)</label>
                  <input 
                    type="text" 
                    value={keywords}
