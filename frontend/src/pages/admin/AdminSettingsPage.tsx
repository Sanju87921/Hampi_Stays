...
export function AdminSettingsPage() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      const promoData = await apiClient.get<any>(
        "/admin/guide-promotion-settings",
      );
      setGuidePromoSettings(promoData);
      const analyticsData = await apiClient
        .get<any>("/admin/guide-promotion-analytics")
        .catch(() => null);
  ...
  const toggleRequirement = async (
    role: "traveller" | "resortOwner" | "guide",
    req: string,
  ) => {
    ...
    <div className="min-h-screen bg-sand-50 pt-32 pb-20 transition-colors">
      <div className="container mx-auto px-4 md:px-8 max-w-4xl">
        <div className="mb-12">
          <Button
            variant="outline"
            className="mb-6 rounded-xl border-sand-200 text-navy-950 font-medium hover:bg-gold-50"
          ...
          <p className="text-navy-950 font-medium">
            Configure global preferences and operational toggles.
          </p>
        </div>
        ...
        <div className="space-y-6">
              ...
              {[
                {
                  ...
                  desc: "Receive alerts when new owners or guides register.",
                },
                {
                  key: "notifyHighValueBookings",
                  title: "High-Value Bookings",
                  desc: "Alerts for bookings exceeding $1000.",
                },
                {
                  key: "notifySystemAlerts",
                  title: "System Alerts",
                  desc: "Critical system performance and security alerts.",
                },
              ].map((item, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex items-center justify-between p-4 border border-sand-100 rounded-xl hover:bg-sand-50 :bg-sand-100/50 transition-colors"
                  >
                    <div>
                    ...
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 rounded-full bg-white transition-transform ${isActive ? "left-7" : "left-1"}`}
                      />
                      {isUpdating && (
                        <Loader2 className="w-4 h-4 animate-spin text-navy-950 absolute -left-6" />
                      )}
                    </button>
                  </div>
                );
          ...
          >
            <div className="mb-6">
              ...
              <p className="text-sm text-navy-950 mt-1">
                Dynamically control which documents are required for onboarding.
              </p>
            </div>
              ...
              <div className="bg-sand-50 rounded-2xl p-5 border border-sand-200">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    const isActive =
                      verificationSettings?.travellerRequirements?.includes(
                        req.id,
                      );
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <div
                            className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${isActive ? "left-4" : "left-0.5"}`}
                          />
                          {isUpdating && (
                            <Loader2 className="w-3 h-3 animate-spin text-navy-950 absolute -left-5" />
                          )}
                        </button>
                      </div>
                    );
              ...
              <div className="bg-sand-50 rounded-2xl p-5 border border-sand-200">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    {
                      id: "TOURISM_REGISTRATION",
                      label: "Tourism Registration",
                    },
                    { id: "FSSAI_LICENSE", label: "FSSAI License" },
                  ].map((req) => {
                    const isActive =
                      verificationSettings?.resortOwnerRequirements?.includes(
                        req.id,
                      );
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          disabled={!verificationSettings || isUpdating}
                          onClick={() =>
                            toggleRequirement("resortOwner", req.id)
                          }
                          className={`w-9 h-5 rounded-full transition-colors relative flex items-center disabled:opacity-50 ${isActive ? "bg-gold-500" : "bg-sand-200"}`}
                        >
                          <div
              ...
              <div className="bg-sand-50 rounded-2xl p-5 border border-sand-200">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    const isActive =
                      verificationSettings?.guideRequirements?.includes(req.id);
              ...
              {[
                {
                  ...
                  title: "Enable Guide Recommendations",
                  desc: "Global toggle for the Smart Guide Discovery system.",
                },
                {
                  key: "enableDashboardBanner",
                  title: "Dashboard Banner",
                  desc: "Show contextual banner on traveler dashboard.",
                },
                {
                  key: "enableSuccessUpsell",
                  title: "Booking Success Upsell",
                  desc: "Prompt travelers to book a guide after resort confirmation.",
                },
                {
                  key: "enableBundleOffers",
                  title: "Stay + Guide Bundles",
                  desc: "Offer discounts when booked together.",
                },
              ].map((item, i) => {
                ...
                return (
                  <div
                    key={i}
                    className="flex items-center justify-between p-4 border border-sand-100 rounded-xl hover:bg-sand-50 transition-colors"
                  >
                    <div>
                      <h4 className="font-bold text-navy-950 text-sm">
                        {item.title}
              ...
              {guidePromoSettings?.enableBundleOffers && (
                <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-sand-100 rounded-xl bg-sand-50 transition-colors">
                  <div className="mb-4 sm:mb-0">
                    ...
                    <p className="text-xs text-navy-950 mt-0.5">
                      Discount applied when booking stay + guide.
                    </p>
                  </div>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setGuidePromoSettings({
                          ...guidePromoSettings,
                          bundleDiscountAmount: Number(e.target.value),
                        })
                      }
                    ...
                    <button
                      ...
                      onClick={() =>
                        togglePromoSetting(
                          ...
                          guidePromoSettings.bundleDiscountAmount,
                        )
                      }
                      className="px-3 py-1.5 bg-navy-950 text-white rounded-lg text-xs font-bold"
                    >
                      {updatingPromo === "bundleDiscountAmount" ? (
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-sand-50 rounded-2xl p-4 border border-sand-200">
                <p className="text-[10px] font-bold text-navy-950/40 uppercase tracking-widest mb-1">
                  Impressions
                </p>
                <p className="text-2xl font-bold text-navy-950">
                  {guideAnalytics?.impressions?.toLocaleString() || 0}
                </p>
              </div>
              <div className="bg-sand-50 rounded-2xl p-4 border border-sand-200">
                <p className="text-[10px] font-bold text-navy-950/40 uppercase tracking-widest mb-1">
                ...
                <p className="text-2xl font-bold text-navy-950">
                  {guideAnalytics?.clicks?.toLocaleString() || 0}
                </p>
              </div>
              <div className="bg-sand-50 rounded-2xl p-4 border border-sand-200">
                <p className="text-[10px] font-bold text-navy-950/40 uppercase tracking-widest mb-1">
                ...
                <p className="text-2xl font-bold text-navy-950">
                  {guideAnalytics?.guideBookings?.toLocaleString() || 0}
                </p>
              </div>
              <div className="bg-gold-50 rounded-2xl p-4 border border-gold-200">
                <p className="text-[10px] font-bold text-gold-800 uppercase tracking-widest mb-1">
                  Revenue Gen
                </p>
                <p className="text-2xl font-bold text-gold-700">
                  ₹{guideAnalytics?.revenueGenerated?.toLocaleString() || 0}
                </p>
              </div>
            </div>
          </motion.div>
          ...
          >
            <div className="mb-6">
              ...
              <p className="text-sm text-navy-950 mt-1">
                Manage mandatory authentication verifications.
              </p>
            </div>
              ...
              {[
                {
                  ...
                  desc: "Require users to verify their phone numbers.",
                },
                {
                  id: "EMAIL_VERIFICATION",
                  label: "Email Verification",
                  desc: "Require users to verify their email addresses.",
                },
              ].map((req) => {
                const isActive =
                  verificationSettings?.travellerRequirements?.includes(req.id);
                const isUpdating = updatingReq === `traveller-${req.id}`;
                return (
                  <div
                    key={req.id}
                    className="bg-sand-50 rounded-2xl p-5 border border-sand-200 flex items-center justify-between"
                  >
                    <div>
                      <h4 className="font-bold text-navy-950 text-sm">
                        {req.label}
          ...
          >
            ...
            <div className="space-y-4">
              <div
                className={`flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-xl border transition-colors ${
              ...
              >
                <div className="mb-4 sm:mb-0">
                  <h4
                    className={`font-bold mb-1 ${settings?.maintenanceMode ? "text-red-900 " : "text-navy-950 "}`}
                  >
                    Maintenance Mode
                  </h4>
                  <p
                    className={`text-xs ${settings?.maintenanceMode ? "text-red-800/70 " : "text-navy-950 "}`}
                  >
                ...
                >
                  {updatingToggle === "maintenanceMode" ? (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-sand-50 rounded-xl border border-sand-200 transition-colors">
                <div className="mb-4 sm:mb-0">
                  ...
                  <p className="text-xs text-navy-950 ">
                    Log all read/write operations for compliance.
                  </p>
                </div>
                <button
                  disabled={
                    !settings || updatingToggle === "detailedAuditLogging"
                  }
