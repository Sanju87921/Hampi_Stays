...
const SPECIAL_REQUEST_OPTIONS = [
  ...
  {
    id: "anniversary",
    label: "Anniversary / Special Occasion Setup",
    icon: Heart,
  },
...
export function CheckoutPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (
        referralRes &&
        referralRes.data &&
        referralRes.data.availableCredits
      ) {
  ...
  const nights = useMemo(() => {
    ...
    return Math.max(
      1,
      Math.ceil((cout.getTime() - cin.getTime()) / (1000 * 60 * 60 * 24)),
    );
  }, [bookingData, hasBookingData]);

  if (!hasBookingData) return <Navigate to="/resorts" replace />;

  const airportPickupCost = selectedRequests.includes("airport_pickup")
    ? 1500
    : 0;
  const basePrice =
    bookingData.baseNightlyPrice ||
    Math.round(bookingData.totalPrice / 1.12 / nights);
  ...
  const mealTotal = selectedMeals.reduce((acc, mealName) => {
    const pkg = (bookingData.mealPackages || []).find(
      (p: any) => p.name === mealName,
    );
    if (pkg) {
  ...
  let finalPrice = Math.max(
    0,
    grandTotal - (appliedCoupon?.discountAmount || 0),
  );
  // Cap credits at finalPrice to prevent negative total — enforced both here (UI) and on backend
  const creditsApplied = useCredits
    ? Math.min(availableCredits, finalPrice)
    : 0;
    ...
    try {
      const allRequests = [
        ...selectedRequests.map(
          (id) => SPECIAL_REQUEST_OPTIONS.find((o) => o.id === id)?.label || id,
        ),
        specialNote,
      ]
        .filter(Boolean)
      ...
      const booking = await apiClient.post<any>("/bookings", {
        ...
        selectedMeals,
        promotionId: appliedCoupon
          ? activePromotions.find((p) => p.code === appliedCoupon.code)?.id
          : null,
      ...
      if (booking.orderId) {
        const razorpayKey =
          import.meta.env.VITE_RAZORPAY_KEY_ID || "rzp_test_Snxno5G3tcQKcs";
        ...
        const options = {
          ...
          image:
            "https://res.cloudinary.com/dfs6lmdns/image/upload/c_fill,g_center,w_512,h_512/v1779876742/hampi-stays/hampistays-logo.png",
          ...
          handler: async function (response: any) {
            try {
              ...
              await apiClient.post(
                `/bookings/${booking.referenceNumber}/verify-payment`,
                {
      ...
      if (errMsg.includes("KYC_REQUIRED")) {
        toast.error(
          "High-value booking limit reached. Please verify your KYC to proceed.",
        );
          ...
          <div className="lg:col-span-8 space-y-6">
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                ...
                >
                  <h2 className="text-2xl font-bold font-serif text-navy-950">
                    Guest Information
                  </h2>
                ...
                >
                  ...
                  <div
                    className={cn(
                      ...
                      addInsurance
                        ...
                        : "border-sand-100 hover:border-green-200",
                    )}
                  ...
                  >
                        ...
                        <div
                          className={cn(
                            ...
                            addInsurance
                              ...
                              : "bg-sand-50 text-navy-950/30",
                          )}
                        >
                          <Shield className="w-7 h-7" />
                        </div>
                        <div>
                          <p className="font-bold text-navy-950 text-lg">
                            Travel Insurance
                          </p>
                          <p className="text-sm text-navy-950/50">
                            Cancel for any reason, medical coverage, trip delay
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-sand-100">
                      <p className="text-xs text-navy-950/40 italic">
                        ~2% of booking value · Provided by HDFC ERGO
                      </p>
                      <p className="font-bold text-green-700">
                        {fmt(insuranceCost || Math.round(nightsTotal * 0.02))}
                      </p>
                    </div>
                  </div>

                  {/* Meal Packages */}
                  {bookingData.mealPackages &&
                    bookingData.mealPackages.length > 0 && (
                      <div className="space-y-4">
                        <h3 className="text-lg font-bold text-navy-950 font-serif">
                          Curated Meal Packages
                        </h3>
                        <div className="grid grid-cols-1 gap-4">
                          {bookingData.mealPackages.map((pkg: any) => {
                            const isSelected = selectedMeals.includes(pkg.name);
                            const totalMealPrice =
                              pkg.price * (bookingData.adults || 1) * nights;
                            return (
                              <div
                                key={pkg.name}
                                className={cn(
                                  ...
                                  isSelected
                                    ...
                                    : "border-sand-100 hover:border-gold-200",
                                )}
                              ...
                              >
                                ...
                                <div className="flex flex-row md:flex-col items-center md:items-end justify-between md:justify-center gap-2 border-t md:border-t-0 pt-4 md:pt-0 border-sand-100">
                                  <div
                                    className={cn(
                                      "w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all order-2 md:order-1",
                                      isSelected
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-navy-950/40">
                            Payment will be charged in INR
                          </p>
                        </div>
                      </div>
                      <div className="flex w-full sm:w-auto rounded-xl overflow-hidden border border-sand-200">
                        {(["INR", "USD", "EUR", "GBP"] as const).map((c) => (
                          <button
                            ...
                            className={cn(
                              "flex-1 sm:flex-none px-4 py-2 text-xs font-bold transition-all",
                              currency === c
                                ? "bg-navy-950 text-white"
                                : "bg-white text-navy-950/40 hover:text-navy-950",
                            )}
                          >
                            {c}
                          </button>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 pt-4">
                    ...
                    <Button
                      className="w-full sm:flex-1 px-12 shadow-gold h-14"
                      onClick={() => setStep(3)}
                    >
                      {appliedCoupon
                        ? "Continue to Payment"
                ...
                >
                  ...
                  <div className="p-6 rounded-3xl bg-sand-50 border border-sand-100 flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-navy-950/50">
                        You will be redirected to Razorpay's secure portal to
                        complete your transaction using Cards, UPI, or
                        NetBanking.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row items-center gap-6 pt-4">
                    <Button
                      size="lg"
                      className="w-full sm:w-auto px-12 h-14 shadow-gold"
                    ...
                    <button
                      onClick={() => setStep(2)}
                      className="text-sm font-bold text-navy-950/40 hover:text-navy-950 transition-colors py-2"
                    >
                      Back to Add-ons
                    </button>
                  </div>
          ...
          <aside className="lg:col-span-4">
            <div className="bg-white rounded-[2.5rem] border border-sand-100 overflow-hidden shadow-luxury sticky top-28">
              ...
              <div className="p-8">
                ...
                <p className="text-sm text-gold-600 font-medium italic mb-6">
                  ...
                  {activePromotions.length > 0 && !appliedCoupon && (
                    <div className="space-y-2 mb-4">
                      {activePromotions.map((promo) => (
                        <div
                          key={promo.id}
                          className="p-4 rounded-xl border border-sand-200 bg-white hover:border-gold-300 transition shadow-sm mb-3"
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div>
                              ...
                              <p className="text-xs text-navy-950/60 mt-1">
                                {promo.description ||
                                  (promo.discountType?.toUpperCase() ===
                                  "PERCENTAGE"
                    ...
                    <div className="p-4 rounded-2xl bg-gold-50/50 border border-gold-200 flex items-center justify-between gap-3 shadow-sm">
                      <div>
                        <p className="text-sm font-bold text-navy-950 flex items-center gap-2">
                          🎉 {appliedCoupon.name}
                          <span className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-bold tracking-widest uppercase">
                            Applied
                          </span>
                        </p>
                        <p className="text-xs text-navy-950/60 mt-1">
                          Discount of {fmt(appliedCoupon.discountAmount)}{" "}
                          applied to total.
                        </p>
                      </div>
                      <button
                        onClick={handleRemoveCoupon}
                        className="text-xs font-bold text-red-600 hover:text-red-700 uppercase tracking-wider underline shrink-0"
                      >
                        Remove
                      </button>
                    </div>
                ...
                <div className="space-y-2 pt-4 border-t border-sand-100 text-sm">
                  <div className="flex justify-between text-navy-950/60">
                    <span>
                      {nights} night{nights > 1 ? "s" : ""} × {fmt(basePrice)}
                    </span>
                  ...
                  {mealTotal > 0 && (
                    <>
                      ...
                      <div className="flex justify-between text-navy-950/60 text-xs">
                        ...
                        <span className="font-bold text-navy-950/70">
                          {fmt(mealTaxes)}
                        </span>
                      </div>
                    </>
                      ...
                      <label className="flex items-center justify-between p-3 rounded-xl border border-gold-200 bg-gold-50/50 cursor-pointer transition-all hover:bg-gold-50">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <span className="text-[10px] text-navy-950/60 font-bold uppercase tracking-widest">
                              Available: {fmt(availableCredits)}
                            </span>
                          </div>
                        </div>
                  ...
                  <div className="flex justify-between pt-3 border-t border-sand-100">
                    <span className="text-lg font-bold text-navy-950 font-serif">
                      Total
                    </span>
                  ...
                  {currency !== "INR" && (
                    <p className="text-[10px] text-navy-950/30 italic text-right">
                      *Approx. rate. Charged in INR
                    </p>
                  )}
                </div>
                ...
                <div className="mt-6 p-4 bg-sand-50 rounded-2xl flex gap-3 items-start">
                  ...
                  <p className="text-xs text-navy-950/50 italic">
